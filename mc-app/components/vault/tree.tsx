...
export function Tree({ node, selected, onSelect, depth }: { node: TreeNode; selected: string | null; onSelect: (p: string) => void; depth: number }) {
  const entries = Array.from(node.children.values()).sort((a, b) => a.name.localeCompare(b.name));
  const files = node.files.slice().sort((a, b) => (b.mtimeMs || 0) - (a.mtimeMs || 0) || (a.__name || "").localeCompare(b.__name || ""));

  return (
    <div className="space-y-1">
      {depth > 0 ? (
        <div className={clsx("flex items-center gap-2 px-2 py-1 text-xs font-semibold text-muted-foreground", depth === 1 ? "mt-1" : "")}
      ...
      {files.map((f) => {
        ...
        return (
          ...
          >
            ...
            <div className={clsx("mt-0.5 truncate text-[11px]", active ? "text-primary-foreground/80" : "text-muted-foreground")}>{f.path}</div>
          </button>
        );
      })}
    </div>
